...
async function getInitialNotification(): Promise<PushPayload | null> {
  ...
  if (response) {
    return (response.notification.request.content.data as unknown as PushPayload) ?? null;
  }
  return null;
}
...
function showWebNotification(title: string, body: string, data?: PushPayload): void {
