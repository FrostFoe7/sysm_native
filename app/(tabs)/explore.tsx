  ...
  const exploreListData: ExploreListItem[] = useMemo(() => {
    ...
    if (exploreFeed.length > 0) {
      return [
        ...
        ...exploreFeed
          .filter(
            (item: RankedExploreItem) =>
              item.content_type === "thread" && item.thread,
          )
  ...
  const handleMore = useCallback(
    (threadId: string) => {
      const allThreads = exploreListData
        .filter(
          (d): d is ExploreListItem & { type: "thread" } => d.type === "thread",
        )
        .map((d) => d.thread);
              ...
              <HStack className="items-center px-4 py-3" space="md">
                ...
                <VStack className="flex-1 overflow-hidden">
                  <HStack className="items-center" space="xs">
                    <Text
                      className="text-[15px] font-bold text-brand-light"
                      numberOfLines={1}
                      style={{ flexShrink: 1 }}
                    >
                  ...
                  {item.user.bio ? (
                    <Text
                      className="mt-0.5 text-[13px] text-[#999]"
                      numberOfLines={1}
                    >
                      {item.user.bio}
                    </Text>
      ...
      <View className="flex-1 lg:flex-row lg:justify-center">
        <View className="flex-1 lg:max-w-[600px]">
          ...
          <View className="sticky top-0 z-10 bg-brand-dark p-4 lg:pt-8">
            <HStack
              className="h-[48px] items-center rounded-xl bg-brand-border px-4"
              space="sm"
            >
