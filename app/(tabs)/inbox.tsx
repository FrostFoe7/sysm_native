...
function InlineChatPanel({ conversationId }: { conversationId: string }) {
  ...
  return (
    <View className="flex-1 border-l border-brand-border bg-brand-dark">
      {/* Header */}
      <HStack
        className="items-center border-b border-brand-border px-4 py-3"
        space="sm"
      >
        ...
        <VStack className="flex-1">
          ...
          {isGroup ? (
            <Text className="text-[11px] text-brand-muted">
              {details.participants.length} members
            </Text>
          ) : typingUsers.length > 0 ? (
            <Text className="text-[11px] italic text-brand-blue">
              typing...
            </Text>
          ) : (
        ...
        <HStack space="sm">
          ...
          {isGroup && (
            <Pressable
              onPress={() =>
                router.push(`/group-info/${conversationId}` as any)
              }
...
export default function InboxScreen() {
  ...
  const renderItem = useCallback(
    ({ item }: { item: ConversationWithDetails }) => (
      <View
        className={
          isDesktop && activeConvId === item.conversation.id ? "bg-white/5" : ""
        }
      >
        <ConversationRow
          conversation={item}
  ...
  const inboxList = (
    <View
      className={
        isDesktop ? "w-[360px] border-r border-brand-border" : "flex-1"
      }
    >
      {/* Header */}
    ...
    return (
      <SafeAreaView className="flex-1 bg-brand-dark" edges={["top"]}>
        <View className="flex-1 flex-row">
          {inboxList}
          {activeConvId ? (
            <InlineChatPanel conversationId={activeConvId} />
          ) : (
  ...
  return <ScreenLayout>{inboxList}</ScreenLayout>;
}
