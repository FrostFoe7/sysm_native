...
serve(async (req) => {
  try {
    ...
    if (message.type === 'system') {
      return new Response(JSON.stringify({ ok: true, skipped: 'system' }), { status: 200 });
    }
    ...
    const { data: targets, error: targetError } = await supabase.rpc('get_push_targets', {
    ...
    if (targetError || !targets || targets.length === 0) {
      return new Response(
        JSON.stringify({ ok: true, skipped: 'no_targets', error: targetError?.message }),
        { status: 200 },
      );
    }
    ...
    if (pushMessages.length === 0) {
      return new Response(JSON.stringify({ ok: true, skipped: 'no_valid_tokens' }), { status: 200 });
    }
