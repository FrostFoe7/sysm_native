...
export function useExploreFeed() {
  ...
  const likeMutation = useMutation({
    ...
    onMutate: async (threadId) => {
      ...
      const previous = queryClient.getQueryData<ExploreItem[]>(['explore-feed']);

      queryClient.setQueryData<ExploreItem[]>(['explore-feed'], (old) =>
        old?.map((item) => {
          if (item.content_type === 'thread' && item.content_id === threadId && item.thread) {
      ...
      const wasLiked = previous?.find((i) => i.content_id === threadId)?.is_liked ?? false;
