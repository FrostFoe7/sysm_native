...
export function FollowersModal({
      ...
      const followerResults = await Promise.all(
        followerUsers.map(async (u) => ({
          ...
          isFollowedByMe: currentUserId
            ? await UserService.isFollowing(u.id)
            : false,
        })),
      );
  ...
  const renderItem = useCallback(
    ({ item }: { item: User & { isFollowedByMe: boolean } }) => {
          ...
          <HStack className="items-center px-4 py-3" space="md">
            ...
            <VStack className="min-w-0 flex-1">
              ...
              <Text className="text-[13px] text-brand-muted" numberOfLines={1}>
                {item.display_name} · {formatCount(item.followers_count)}{" "}
                followers
              </Text>
            </VStack>
            {!isSelf && (
              ...
              >
                <ButtonText
                  className={`text-[13px] font-semibold ${item.isFollowedByMe ? "text-brand-light" : "text-black"}`}
                >
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          ItemSeparatorComponent={() => (
            <Divider className="ml-[68px] bg-brand-border" />
          )}
          ListEmptyComponent={
